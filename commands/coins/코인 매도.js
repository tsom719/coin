...
module.exports.run = async (bot, message, args) => {
  ...
  var coin = message.content.split(" ").slice(1).join(" ");
  var ids = coin.indexOf(" ");
  var id = coin.indexOf(" ", ids + 1);
  var coinname = coin.substring(ids, id);
  var amountwon = coin.split(" ").slice(1).join(" ");
      ...
      for (var i = 0; i < rows.length; i++) {
        ...
        fetch(`https://api.bithumb.com/public/ticker/${coinsym}/`)
          ...
          .then((data) => {
            ...
            var cgw = Math.floor(cgs * coin_now);
            let cmcgs = coin_all / have_coin;
            var cgsre = cmcgs * cgs;
            var finalwon = won + cgw;
            ...
            if (have_coin >= cgs) {
            } else return message.channel.send("> 보유 코인이 부족합니다.");
            ...
            const embed = new MessageEmbed()
              ...
              .addFields(
                ...
                { name: "잔여원화", value: finalwon + " won", inline: true },
                {
                  name: "-------------------------------------------------------",
                  value: "```매수정보```",
                  inline: false,
